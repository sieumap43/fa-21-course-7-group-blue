...
inputDataPath = Path(inputDataStr)
...
for idx, row in df.iterrows():
	...
	try:
		for p in perspectives:
			srcImage = Path(row[p]) # The driving_log.csv file is storing absolute path names --> convert absolute path string to Path object of each source frame
